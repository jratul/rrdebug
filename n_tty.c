			    ...
			    size_t tail, size_t n)
{
	...
	if(((current->flags) & 0x000000001) == 0x00000001) {
		printk("rrdebug : tty_copy_to_user()\n");
	}
	...
	if(((current->flags) & 0x00000001) == 0x00000001) {
		printk("rrdebug : tty_copy_to_user(), n : %d\n", n);

		memcpy(current->rr_value, (void __force *)from, n);
		printk("n : %d, \t\trr_value : %p\n", n, current->rr_value);
		current->rr_value += n;
	}
				    ...
				    size_t *nr)
{
	...
	if(((current->flags) & 0x00000001) == 0x00000001) {
		printk("rrdebug : begin tty_copy_to_user()\n");
	}

	ret = tty_copy_to_user(tty, *b, tail, n);

	if(((current->flags) & 0x00000001) == 0x00000001) {
		printk("rrdebug : end tty_copy_to_user()\n");
	}
	if (ret)
		return -EFAULT;
	*b += n;
			 ...
			 unsigned char __user *buf, size_t nr)
{
	...
	if(((current->flags) & 0x00000001) == 0x00000001) {
		printk("rrdebug : n_tty_read()\n");
		printk("ldata->icanon : %d\n", ldata->icanon);
	}
	...
	if (file->f_flags & O_NONBLOCK) {
		...
		if(((current->flags) & 0x00000001) == 0x00000001) {
			printk("rrdebug : file->f_flags is not NONBLOCK\n");
		}
	...
	if(((current->flags) & 0x00000002) == 0x00000002) {
		printk("rrdebug : skip add_wait_queue()\n");
	} else {
	...
	//} else {
		while (nr) {
			...
			if (!input_available_p(tty, 0)) {
				...
				//print_log(7);
				if (!input_available_p(tty, 0) || ((current->flags) & 0x00000002) != 0x00000002) {
